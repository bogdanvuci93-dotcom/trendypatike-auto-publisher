...
old_anchor = '''function normalizeHeadlineGroup(lines,maxLines){ const cleaned=(lines||[]).map(line=>({text:simplifyKidPhrase(line?.text),accent:line?.accent===true})).filter(line=>line.text); if(!cleaned.length)return [{text:"PATIKA IMA ZANIMLJIVU PRIČU",accent:false}]; return capHeadlineWords(cleaned.slice(0,maxLines),18); }'''
long_anchor = '''function normalizeHeadlineGroup(lines,maxLines){ const cleaned=(lines||[]).map(line=>({text:dedupeAdjacentWords(simplifyKidPhrase(line?.text)),accent:line?.accent===true})).filter(line=>line.text); if(!cleaned.length)return [{text:"PATIKA IMA ZANIMLJIVU PRIČU",accent:false}]; return capHeadlineWords(cleaned.slice(0,maxLines),40); }'''
...
function semanticAccentSegments(lines,maxWords=40){
  ...
  for(const line of source){
    ...
    const flush=(accent)=>{
      ...
      out.push({text,accent:accent===true && count<=5 && meaningful});
      buffer=[];
    };
    for(let i=0;i<words.length;i++){
      ...
      if(isImportantNumberToken(word)){
        ...
        if(/^(MILIONA?|MILIJARDE?|PARA|PAROVA|KOMADA|GODINA|RSD|ZAPOSLENIH)$/.test(next)){
          numberText += ` ${words[++i]}`;
        }
...
function normalizeHeadlineGroup(lines,maxLines){
  const cleaned=(lines||[]).map(line=>({text:dedupeAdjacentWords(normalizeMoneyNotation(line?.text)),accent:line?.accent===true})).filter(line=>line.text);
  if(!cleaned.length)return [{text:"PATIKA IMA ZANIMLJIVU PRIČU",accent:false}];
  return semanticAccentSegments(cleaned.slice(0,maxLines),40);
}'''

if replacement not in s:
    if long_anchor in s:
        s = s.replace(long_anchor, replacement, 1)
    elif old_anchor in s:
        s = s.replace(old_anchor, replacement, 1)
    else:
        raise SystemExit("Semantic accent patch failed: compatible normalizeHeadlineGroup anchor not found")
...
print("Semantic accents applied: important numbers green, money uses $ / € / RSD notation, long-copy helper preserved.")
