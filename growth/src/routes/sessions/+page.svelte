<script lang="ts">
  export let data: {
    rows: {
      ...
      events:number; pageViews:number; productViews:number; addToCart:number; cartViews:number; checkout:number; rageClicks:number; deadClicks:number; activeMs:number; maxScroll:number; exitStage:string;
    }[];
  };
...
<header>
  ...
  <h1>Sessions</h1>
  <p>Putanja, aktivno vreme, poslednja stranica, scroll, ATC/checkout i problematični klikovi.</p>
</header>
...
<section class="summary">
  ...
  <div><span>Problem clicks</span><b>{data.rows.filter((r)=>r.rageClicks+r.deadClicks>0).length}</b></div>
</section>

<section class="panel">
  {#if data.rows.length}
    <div class="table-wrap"><table>
      <thead><tr><th>Session</th><th>Source</th><th>Landing → Exit</th><th>Active</th><th>PDP</th><th>ATC</th><th>Checkout</th><th>Max scroll</th><th>Exit stage</th><th>Issues</th></tr></thead>
      <tbody>
        {#each data.rows as row}
          <tr>
            <td><a href={`/sessions/${row.id}`}><strong>{new Date(row.startedAt).toLocaleString('sr-RS')}</strong><small>Open detailed journey →</small></a></td>
...
<style>
  ...
  .summary{display:grid;grid-template-columns:repeat(4,1fr);gap:10px;margin-bottom:12px}.summary div,.panel{background:#101318;border:1px solid #20242b;border-radius:16px;padding:16px}.summary span{display:block;color:#7f8997;font-size:11px;margin-bottom:8px}.summary b{font-size:20px}.panel{padding:20px}.table-wrap{overflow:auto}table{width:100%;border-collapse:collapse;font-size:12px}th,td{text-align:left;padding:13px 10px;border-bottom:1px solid #1e232a;white-space:nowrap}th{color:#77818f;font-size:10px;text-transform:uppercase;letter-spacing:.08em}td:first-child{min-width:205px}td:nth-child(3){min-width:300px;max-width:390px;overflow:hidden;text-overflow:ellipsis}small{display:block;color:#65707e;margin-top:4px;font-size:10px}code{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;color:#bcc5d2}.hot{color:#ff9a88;font-weight:800}.stage{font-size:10px;padding:5px 7px;border-radius:99px;background:#262b31;color:#aab2bd}.stage.product{background:#302a16;color:#e3cb6d}.stage.cart,.stage.checkout{background:#321b1d;color:#ff9d9d}.empty{padding:18px;background:#0c0f13;border:1px dashed #2a313a;border-radius:14px;color:#8e98a7}@media(max-width:850px){.summary{grid-template-columns:repeat(2,1fr)}}
</style>
