<script lang="ts">
  export let data: {
    ...
    rows: { accountId:string; accountName:string; currency:string; campaignId:string; adsetId:string; adId:string; name:string; spend:number; impressions:number; clicks:number; purchases:number; revenue:number; ctr:number; cpc:number; roas:number; cpa:number }[];
...
{#if data.mixedCurrency}
  <div class="warning">Povezano je više Meta ad naloga sa različitim valutama. Ukupan spend/revenue se ne sabiraju pouzdano dok ne izaberemo primarni TrendyPatike ad account.</div>
{/if}

<section class="metrics">
  <div><span>Spend</span><b>{data.mixedCurrency ? 'Mixed' : money(data.totals.spend)}</b></div>
...
<section class="panel">
  ...
  {#if data.rows.length}
    <div class="table-wrap"><table>
      <thead><tr><th>Ad</th><th>Ad account</th><th>Spend</th><th>CTR</th><th>CPC</th><th>Purchases</th><th>Revenue</th><th>ROAS</th><th>CPA</th></tr></thead>
