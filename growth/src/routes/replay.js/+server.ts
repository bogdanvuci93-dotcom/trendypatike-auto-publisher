...
const SOURCE = String.raw`(() => {
  ...
  var queue=[],sending=false,stopped=false,lastPointerAt=0,lastScrollAt=0,lastSnapshotAt=0,lastHtml='',snapshotCount=0,dirty=true,idleTimer=0,lastViewport='';
  ...
  var viewport=function(force){var key=innerWidth+'x'+innerHeight+'@'+(devicePixelRatio||1);if(!force&&key===lastViewport)return;lastViewport=key;push({kind:'viewport',t:Date.now(),vw:innerWidth,vh:innerHeight,dpr:devicePixelRatio||1,device:device(),path:location.pathname||'/'});};
  ...
  var snapshot=function(force){
    ...
    var now=Date.now();if(!force&&(!dirty||now-lastSnapshotAt<5000))return;
    var html=sanitize();lastSnapshotAt=now;if(!html||html.length>235000){dirty=false;return;}if(!force&&html===lastHtml){dirty=false;return;}
    lastHtml=html;dirty=false;snapshotCount++;push({kind:'snapshot',t:now,html:html,path:location.pathname||'/',vw:innerWidth,vh:innerHeight,x:scrollX,y:scrollY});
  };
  var scheduleSnapshot=function(force,delay){if(stopped||snapshotCount>=3||(!importantPage()&&!force))return;clearTimeout(idleTimer);idleTimer=setTimeout(function(){var run=function(){snapshot(Boolean(force));};try{if('requestIdleCallback' in window)window.requestIdleCallback(run,{timeout:1200});else setTimeout(run,0);}catch(e){setTimeout(run,0);}},Math.max(0,delay||0));};
  var flush=function(){if(sending||!queue.length||stopped)return;sending=true;var batch=queue.splice(0,Math.min(queue.length,80));var seq=(batch[0]&&batch[0].t)||Date.now();fetch(ENDPOINT,{method:'POST',mode:'cors',credentials:'omit',keepalive:true,headers:{'content-type':'application/json'},body:JSON.stringify({sessionId:sid,seq:seq,events:batch})}).then(function(r){return r.json().catch(function(){return {};}).then(function(p){return {ok:r.ok,p:p};});}).then(function(res){if(res.p&&res.p.capped)stopped=true;else if(!res.ok)queue=batch.concat(queue).slice(-160);}).catch(function(){queue=batch.concat(queue).slice(-160);}).finally(function(){sending=false;});};
  ...
  viewport(true);
  addEventListener('resize',function(){viewport(false);},{passive:true});
  addEventListener('orientationchange',function(){setTimeout(function(){viewport(true);},200);},{passive:true});
  ...
  var routeChanged=function(){dirty=true;snapshotCount=0;push({kind:'route',t:Date.now(),path:location.pathname||'/'});viewport(true);scheduleSnapshot(false,1200);};
  ...
  push({kind:'route',t:Date.now(),path:location.pathname||'/'});
  scheduleSnapshot(false,1400);
  setInterval(flush,5000);
  window.__TP_REPLAY_STATUS__={version:'2026-09-10.1',endpoint:ENDPOINT,sessionId:sid,mode:'priority-idle-mobile-exact'};
})();`;

export const GET: RequestHandler = async () => new Response(SOURCE, {
  headers: {'Content-Type':'application/javascript; charset=utf-8','Cache-Control':'no-store, max-age=0','Cross-Origin-Resource-Policy':'cross-origin','Access-Control-Allow-Origin':'*'}
});
