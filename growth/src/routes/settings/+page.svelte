<script lang="ts">
  export let data: {
    ...
    autosync: { intervalMinutes:number; state:Record<string,{lastSuccessAt:number;lastAttemptAt:number;lastError:string;result:Record<string,unknown>}> };
  ...
  async function runSync(provider: 'shopify' | 'meta') {
    if (provider === 'shopify') { syncingShopify = true; shopifySync = 'Syncing…'; }
    else { syncingMeta = true; metaSync = 'Syncing…'; }
    try {
...
<header>
  ...
  <h1>Connections</h1>
  <p>Shopify, Meta Ads i storefront tracker. Tokeni se čuvaju šifrovano u D1.</p>
</header>
...
<section class="checks">
  ...
  <div class="ok">AUTO SYNC <b>EVERY {data.autosync.intervalMinutes} MIN</b></div>
</section>
...
<div class="grid">
  <article>
    ...
    <p>Validne porudžbine i proizvodi. Otkazane, refunded/partially-refunded, voided i vraćene porudžbine se ne računaju u revenue/order metrike.</p>
    {#if connected('shopify')}
      <div class="health"><b>Auto-sync:</b> {when(data.autosync.state.shopify?.lastSuccessAt || 0)}{#if data.autosync.state.shopify?.lastError}<small>{data.autosync.state.shopify.lastError}</small>{/if}</div>
      <div class="connected-actions"><button on:click={() => runSync('shopify')} disabled={syncingShopify}>{syncingShopify ? 'Syncing…' : 'Sync Shopify now'}</button>{#if shopifySync}<div class:syncerror={shopifySync.startsWith('Greška')} class="syncstatus">{shopifySync}</div>{/if}</div>
    {:else if data.configured.shopify && data.configured.db && data.configured.encryption}
      <form action="/connect/shopify" method="get"><label for="shop">Shopify shop domain</label><div class="row"><input id="shop" name="shop" bind:value={shop} placeholder="trendypatike.myshopify.com" required /><button>Connect Shopify</button></div></form>
  ...
  <article>
    ...
    <p>Poslednjih 7 dana po danu: campaign, ad set, ad, spend, CTR, CPC, purchases i ROAS. Veza ostaje read-only.</p>
    {#if connected('meta')}
      <div class="health"><b>Auto-sync:</b> {when(data.autosync.state.meta?.lastSuccessAt || 0)}{#if data.autosync.state.meta?.lastError}<small>{data.autosync.state.meta.lastError}</small>{/if}</div>
      <div class="connected-actions"><button on:click={() => runSync('meta')} disabled={syncingMeta}>{syncingMeta ? 'Syncing…' : 'Sync Meta Ads now'}</button>{#if metaSync}<div class:syncerror={metaSync.startsWith('Greška')} class="syncstatus">{metaSync}</div>{/if}</div>
    {:else if data.configured.meta && data.configured.db && data.configured.encryption}<a class="button" href="/connect/meta">Connect Meta Ads</a>{:else}<div class="pending">Cloudflare secrets još nisu podešeni.</div>{/if}
  </article>
</div>

<section class="tracker-card">
  <div class="tracker-head"><div><span class="eyebrow">STOREFRONT TRACKER</span><h2>Sessions + clicks + scroll + funnel</h2></div><strong class:on={data.tracker.sessions24h>0}>{data.tracker.sessions24h>0 ? `${data.tracker.sessions24h} sessions / 24h` : 'WAITING FOR INSTALL'}</strong></div>
  <p>Tracker je live. Poslednji signal: <b>{data.tracker.lastSeenAt ? when(data.tracker.lastSeenAt) : '—'}</b>. Ne čitamo input vrednosti, email, telefon ni adresu.</p>
  <div class="snippet"><code>{trackerSnippet}</code><button on:click={copyTracker}>Copy</button></div>
  <div class="tracker-features">Page/PDP · dwell time · Add to cart · Checkout · click target · scroll · rage/dead clicks · UTM/fbclid</div>
</section>

<section class="note"><b>Automatika:</b> Cloudflare cron osvežava Shopify i Meta na 15 minuta. Ručna Sync dugmad ostaju samo kao fallback/test. Meta write akcije ostaju zaključane.</section>
