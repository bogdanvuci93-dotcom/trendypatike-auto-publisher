<script lang="ts">
  export let data: {
    ...
    connectionHealth: Record<string,{ok:boolean;error:string}>;
  ...
  let shop = 'trendypatike.myshopify.com';
  let shopifySync = '';
  let metaSync = '';
  let syncingShopify = false;
  let syncingMeta = false;

  const exists = (provider: string) => data.connections.some((c) => c.provider === provider);
  const healthy = (provider: string) => Boolean(data.connectionHealth?.[provider]?.ok);
  const needsReconnect = (provider: string) => exists(provider) && !healthy(provider);
...
<header><div class="eyebrow">TRENDYPATIKE · 0 € MODE</div><h1>Podešavanja</h1><p>Ovde proveravaš veze i tracker. Posle promene encryption key-a stare OAuth veze moraju jednom ponovo da se autorizuju.</p></header>
{#if data.connected}<div class="success">✓ {data.connected === 'shopify' ? 'Shopify' : 'Meta Ads'} je uspešno ponovo povezan.</div>{/if}
...
<div class="grid">
  <article>
    <div class="top"><div><span>SHOPIFY</span><h2>TrendyPatike Store</h2></div><strong class:on={healthy('shopify')} class:reconnect={needsReconnect('shopify')}>{healthy('shopify') ? 'POVEZAN' : needsReconnect('shopify') ? 'PONOVO POVEŽI' : 'NIJE POVEZAN'}</strong></div>
    ...
    {:else if data.configured.shopify && data.configured.db && data.configured.encryption}
      {#if needsReconnect('shopify')}<div class="reconnect-note">Encryption key je promenjen. Stari Shopify token više ne može da se dešifruje. Ovo je očekivano — samo autorizuj Shopify ponovo.</div>{/if}
      <form action="/connect/shopify" method="get"><label for="shop">Shopify shop domain</label><div class="row"><input id="shop" name="shop" bind:value={shop} required /><button>{needsReconnect('shopify') ? 'Ponovo poveži Shopify' : 'Poveži Shopify'}</button></div></form>
    {:else}<div class="pending">Cloudflare secrets još nisu podešeni.</div>{/if}
  </article>

  <article>
    <div class="top"><div><span>META ADS</span><h2>Facebook / Instagram Ads</h2></div><strong class:on={healthy('meta')} class:reconnect={needsReconnect('meta')}>{healthy('meta') ? 'POVEZAN' : needsReconnect('meta') ? 'PONOVO POVEŽI' : 'NIJE POVEZAN'}</strong></div>
    ...
    {:else if data.configured.meta && data.configured.db && data.configured.encryption}
      {#if needsReconnect('meta')}<div class="reconnect-note">Encryption key je promenjen. Stari Meta token više ne može da se dešifruje. Ponovna OAuth prijava će upisati novi token sa novim key-em.</div>{/if}
      <a class="button" href="/connect/meta">{needsReconnect('meta') ? 'Ponovo poveži Meta Ads' : 'Poveži Meta Ads'}</a>
    {:else}<div class="pending">Cloudflare secrets još nisu podešeni.</div>{/if}
  </article>
</div>
...
<details class="manual"><summary>Ručni sync — test/fallback</summary><div class="manual-actions"><button on:click={() => runSync('shopify')} disabled={syncingShopify || !healthy('shopify')}>{syncingShopify ? 'Osvežavam…' : 'Shopify sada'}</button><button on:click={() => runSync('meta')} disabled={syncingMeta || !healthy('meta')}>{syncingMeta ? 'Osvežavam…' : 'Meta sada'}</button></div>{#if shopifySync}<div class:syncerror={shopifySync.startsWith('Greška')} class="syncstatus">{shopifySync}</div>{/if}{#if metaSync}<div class:syncerror={metaSync.startsWith('Greška')} class="syncstatus">{metaSync}</div>{/if}</details>
<section class="note"><b>Bitno:</b> promena `APP_ENCRYPTION_KEY` ne kvari D1 podatke niti tracker. Menja samo ključ kojim su OAuth tokeni zaključani, zato Shopify i Meta moraju jednom ponovo da se povežu.</section>
